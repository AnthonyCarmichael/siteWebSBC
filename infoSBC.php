...
?>
    ...
    <form class="flex wrap" action="infoSBC.php" method="post" id="filtre">
        ...
        <input class="col-4" type="text" placeholder="Recherche par modéle" name="modele" id="modele">
    ...
    <?php if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'filtre') {
                ...
                if (empty($SBCObjArray))
                    ...
                    <section class="flex wrap">
                        <?php foreach($SBCObjArray as $SBCObj){?>
                            <article class="col-4">
                                <div>                
                                    <a href="img/<?=$SBCObj->get_modele();?>.jpg"><img src="img/<?=$SBCObj->get_modele();?>.jpg" alt="<?=$SBCObj->get_modele();?>.jpg"></a>
                                </div>
                
                                <div>
                                    <p><?=$SBCObj->get_marqueSBC(). ' ' .$SBCObj->get_modele(). ' ' .$SBCObj->get_RAM() . ' Go ' . $SBCObj->get_longueur() . ' par '. $SBCObj->get_largeur() . ' mm';?></p>
                ...
                <?php foreach($SBCs as $SBC){?>
                    <article class="col-4">
                        <div>                
                            <a href="img/<?=$SBC->get_modele();?>.jpg"><img src="img/<?=$SBC->get_modele();?>.jpg" alt="<?=$SBC->get_modele();?>.jpg"></a>
                        </div>
        
                        <div>
                            <p><?=$SBC->get_marqueSBC(). ' ' .$SBC->get_modele(). ' ' .$SBC->get_RAM() . ' Go ' . $SBC->get_longueur() . ' par '. $SBC->get_largeur() . ' mm';?></p>
